...
//key:关键字 path:缓存地址
+ (NSArray *) parseListWithDic:(NSDictionary *)jsonDic path:(NSString *)path key:(NSString *)key page:(NSInteger)page
{
    //json解析
    if ([GlobalConfig isKindOfNSDictionaryClassAndCountGreaterThanZero:jsonDic]) {
        NSArray *responseArray = [jsonDic objectForKey:key];
        if ([GlobalConfig isKindOfNSArrayClassAndCountGreaterThanZero:responseArray]) {
            if (page == 1) {
            //有数据 存缓存 返回array
                [CacheHanding writeCache:path data:responseArray];
            }
            else {
                NSArray *array =  [NSArray arrayWithContentsOfFile:[CacheHanding filepath:path]];
                NSMutableArray *array1 = [NSMutableArray arrayWithArray:array];
                [array1 addObjectsFromArray:responseArray];
                [CacheHanding writeCache:path data:array1];
