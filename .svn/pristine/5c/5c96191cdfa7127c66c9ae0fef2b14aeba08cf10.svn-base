			...
			if (curWidth > constrainedSize.width) {
				...
				for (NSUInteger j = glyphIdx, cj = rowIdx; j < glyphLen; j++, cj++) {
					...
					if (newWidth > constrainedSize.width && j > glyphIdx) {
						...
						} else if (lastRowSpace == 0 || lineBreakMode == NSLineBreakByCharWrapping || (lastLine && (lineBreakMode == NSLineBreakByTruncatingTail || lineBreakMode == NSLineBreakByTruncatingMiddle || lineBreakMode == NSLineBreakByTruncatingHead))) {
...
- (CGSize)sizeWithZFont:(ZFont *)font {
	CGSize size = drawOrSizeTextConstrainedToSize(NO, self, font, CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX),NSLineBreakByClipping, NSTextAlignmentLeft, YES);
...
- (CGSize)sizeWithZFont:(ZFont *)font constrainedToSize:(CGSize)size lineBreakMode:(UILineBreakMode)lineBreakMode {
	size = drawOrSizeTextConstrainedToSize(NO, self, font, size, lineBreakMode, NSTextAlignmentLeft, NO);
	return CGSizeMake(ceilf(size.width), ceilf(size.height));
}
...
- (CGSize)drawAtPoint:(CGPoint)point forWidth:(CGFloat)width withZFont:(ZFont *)font lineBreakMode:(UILineBreakMode)lineBreakMode {
	return drawTextInRect((CGRect){ point, { width, font.leading } }, self, font, lineBreakMode, NSTextAlignmentLeft, YES);
}
...
- (CGSize)drawInRect:(CGRect)rect withZFont:(ZFont *)font lineBreakMode:(UILineBreakMode)lineBreakMode {
	return [self drawInRect:rect withZFont:font lineBreakMode:lineBreakMode alignment:NSTextAlignmentLeft];
}
